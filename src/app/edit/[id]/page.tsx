...
export default function EditBlog() {
  ...
  useEffect(() => {
    if (currentBlog) {
      ...
      if (currentBlog.author._id !== user?._id && user?.role !== 'admin') {
        router.push('/')
        return
      }
      ...
      setFormData({
        ...
        category: currentBlog.category?._id || currentBlog.category || '',
        tags: currentBlog.tags?.map((tag: any) => tag.name).join(', ') || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dispatch(updateBlog({ id: blogId, blogData }))
